              ...
              <div class="card-header" style="background: #ddd;">  
                <div class="row">  
                  ...
                  <div class="col-md-3 text-right"> 
                    ...
                    <a href="{{ route('roles.add') }}" class="btn btn-primary waves-effect waves-light"><i class="fe-plus-square"></i> เพิ่ม </a> 
                  </div>
                </div>
              </div>
              <div class="card-body">  
                <div class="h5"> ตารางข้อมูลบทบาทผู้ใช้งานประจำวันที่ {{ date('m/d/Y') }} </div> 
...
<script> 
  ...
  function datatable(keywrod, status){  
    var table = $('#tbl-roles').DataTable({
      ...
      "lengthChange": true,  
      ajax: {
        url:"{{ route('datatable.permissions') }}", 
        data:{
          keywrod: keywrod, 
          status: status,
        }
