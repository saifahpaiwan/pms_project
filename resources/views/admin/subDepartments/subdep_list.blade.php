              ...
              <div class="card-header" style="background: #ddd;">  
                <div class="row">  
                  ...
                  <div class="col-md-3 text-right"> 
                    ...
                    <a href="{{ route('sub.departments.add') }}" class="btn btn-primary waves-effect waves-light"><i class="fe-plus-square"></i> เพิ่ม </a> 
                  </div>
                </div>
              </div>
              <div class="card-body">  
                <div class="h5"> ตารางข้อมูลแผนกย่อยประจำวันที่ {{ date('m/d/Y') }} </div> 
                <div class="table-responsive"> 
                  <table class="table table-borderless dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;" id="tbl-sub-departments">
                    <thead>
                      <tr> 
...
<script> 
    ...
    Swal.fire({
        ...
        if (result.value) {  
          ...
          .done(function(data, status, error){   
            if(error.status==200){     
              if(data==true){
                $('#tbl-sub-departments').DataTable().destroy();
                datatable(null, null);
              } 
            }
          })
  ...
  $(document).on('click', '#search', function(event) { 
    ...
    $('#tbl-sub-departments').DataTable().destroy();
    datatable(keywrod, status, departments_id);
  });
  ...
  datatable();
  function datatable(keywrod, status, departments_id){  
    var table = $('#tbl-sub-departments').DataTable({
      "processing":false,  
      "serverSide":false,  
      "searching": false,
      "lengthChange": true,  
      ajax: {
        url:"{{ route('datatable.sub.departments') }}", 
        data:{
          keywrod: keywrod, 
          status: status,
