          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-header" style="background: #ddd;">  
                <div class="row">   
                  <div class="col-md-6"> 
                    <div class="m-0 h5"> แก้ไขข้อมูลแผนกย่อย </div>
                  </div>
                  <div class="col-md-6 text-right"> 
                    <a href="{{ route('sub.departments.list') }}" class="m-0 h5"><i class="fe-chevron-left"></i> ย้อนกลับ </a> 
                  </div>  
                </div>
              </div>
              <div class="card-body">  
                ...
                @endif  
                <form method="POST" action="{{ route('save.sub.departments') }}" id="form" enctype="multipart/form-data">
                  @csrf  
                  <input type="hidden" id="statusData" name="statusData" value="U">
                  <input type="hidden" id="id" name="id" value="{{ $data['sub_department_find']->id }}">
                  ...
                  <div class="row"> 
                    <div class="col-md-6 form-group"> 
                      ...
                      value="{{ $data['sub_department_find']->name }}"
                      ...
                      <select id="departments_id" name="departments_id" class="form-control mb-1 @error('departments_id') invalid @enderror" data-toggle="select2" required>
                        ...
                        @if(isset($data['Query_department']))
                          @foreach($data['Query_department'] as $row)
                            <option @if($data['sub_department_find']->departments_id==$row->id) {{ __('selected') }} @endif value="{{ $row->id }}"> {{ $row->name }} </option>
                          @endforeach
                        @endif 
                      </select>   
                    ...
                    <div class="col-md-6 form-group"> 
                      ...
                      <div class="cc-selector"> 
                        <input id="status1" type="radio" name="status" value="0" {{ $data['sub_department_find']->deleted_at == true ? $data['sub_department_find']->deleted_at == 0 ? "checked" : ""  : "checked"  }}/>
                        <label class="drinkcard-cc bg-success" for="status1"> เปิดการแสดงผล </label>  

                        <input id="status2" type="radio" name="status" value="1" {{ $data['sub_department_find']->deleted_at == 1 ? "checked" : "" }}/>
                  ...
                  <div class="row">  
                    <div class="col-md-12 form-group text-right">   
                      <a href="{{ route('sub.departments.list') }}" class="btn btn-lg btn-secondary waves-effect waves-light"><i class="fe-chevron-left"></i> ย้อนกลับ </a> 
                      @if(session('session_close'))
                        @if(session('session_close')=="Y")
                          <button type="button" class="btn btn-lg btn-danger waves-effect waves-light" id="close" data-id="{{ $data['sub_department_find']->id }}"><i class="fe-trash"></i> ยกเลิกข้อมูล </button>
                        @endif
                      @endif 
...
<script> 
  ...
  $(document).on('click', '#close', function(event) { 
    ...
    Swal.fire({
        ...
        if (result.value) {  
          $.post("{{ route('close.sub.departments') }}", {
            _token: "{{ csrf_token() }}",  
            id: id, 
          })
          .done(function(data, status, error){   
            if(error.status==200){     
              if(data==true){
                location.href = "{{ route('sub.departments.list') }}";
              } 
            }
          })
