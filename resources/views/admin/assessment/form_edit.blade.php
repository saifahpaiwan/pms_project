          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-header" style="background: #ddd;">  
                <div class="row">   
                  <div class="col-md-6"> 
                    <div class="m-0 h5"> แก้ไขหัวข้อแบบประเมิน </div>
                  </div>
                  <div class="col-md-6 text-right"> 
                    <a href="{{ route('assessmentform.list') }}" class="m-0 h5"><i class="fe-chevron-left"></i> ย้อนกลับ </a> 
                  </div>   
                </div>
              </div>
              <div class="card-body">  
                ...
                <form method="POST" action="{{ route('save.assessmentform') }}" id="form" enctype="multipart/form-data">
                  ...
                  <input type="hidden" id="id" name="id" value="{{ $data['assessmentForm_find']->id }}">
                  <div class="row"> 
                    <div class="col-md-5"> 
                      <div class="row"> 
                        <div class="col-md-12 form-group"> 
                          ...
                          value="{{ $data['assessmentForm_find']->title }}"
                        ...
                        <div class="col-md-12 form-group"> 
                          ...
                          value="{{ $data['assessmentForm_find']->sub_title }}"
                        ...
                        <div class="col-md-4 form-group"> 
                          ...
                          value="{{ $data['assessmentForm_find']->sum_average }}"
                        ...
                        <div class="col-md-8 form-group text-right"> 
                          ...
                          <div class="cc-selector"> 
                            <input id="status1" type="radio" name="status" value="0" {{ $data['assessmentForm_find']->deleted_at == true ? $data['assessmentForm_find']->deleted_at == 0 ? "checked" : ""  : "checked"  }}/>
                            <label class="drinkcard-cc bg-success" for="status1"> เปิดการแสดงผล </label>  

                            <input id="status2" type="radio" name="status" value="1" {{ $data['assessmentForm_find']->deleted_at == 1 ? "checked" : "" }}/>
                  ...
                  <div class="row">  
                    <div class="col-md-12 form-group text-right">   
                      <a href="{{ route('assessmentform.list') }}" class="btn btn-lg btn-secondary waves-effect waves-light"><i class="fe-chevron-left"></i> ย้อนกลับ </a>  
                      @if(session('session_close'))
                        @if(session('session_close')=="Y")
                          <button type="button" class="btn btn-lg btn-danger waves-effect waves-light" id="close" data-id="{{ $data['assessmentForm_find']->id }}"><i class="fe-trash"></i> ยกเลิกข้อมูล </button>
                        @endif
                      @endif 
...
<script> 
  ...
  function datatable(){  
    ...
    var table = $('#tbl-data').DataTable({
      ...
      "lengthChange": true,  
      ajax: {
        url:"{{ route('datatable.assessmentform.detail') }}", 
        data:{
          id: id,  
        }
      },    
  ...
  $(document).on('click', '#add-row', function(event) { 
    ...
    buttonMg='<input type="hidden" name="array['+count+'][id]" value="0"><button type="button" class="btn btn-icon waves-effect waves-light btn-danger delete-row" data-id="0"> <i class="fas fa-times"></i> </button>';
    table.row.add({
      "buttonMg" : buttonMg,
  ...
  $(document).on('click', '.delete-row', function(event) {
    ...
    Swal.fire({
        ...
        if (result.value) {  
          $.post("{{ route('close.assessmentform.detail') }}", {
            _token: "{{ csrf_token() }}",  
            id: id, 
          })
  ...
  $(document).on('click', '#close', function(event) { 
    ...
    Swal.fire({
        ...
        if (result.value) {  
          $.post("{{ route('close.assessmentform') }}", {
            _token: "{{ csrf_token() }}",  
            id: id, 
          })
          .done(function(data, status, error){   
            if(error.status==200){     
              if(data==true){
                location.href = "{{ route('assessmentform.list') }}";
              } 
            }
          })
          .fail(function(xhr, status, error) { 
