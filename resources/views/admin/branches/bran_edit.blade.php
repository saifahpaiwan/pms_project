            ...
            <div class="card">
              ...
              <div class="card-body">  
                ...
                <form method="POST" action="{{ route('save.branches') }}" id="form" enctype="multipart/form-data">
                  ...
                  <input type="hidden" id="id" name="id" value="{{ $data['branch_find']->id }}">
                  ...
                  <div class="row"> 
                    ...
                    <div class="col-md-6 form-group"> 
                      ...
                      <div class="cc-selector"> 
                        <input id="status1" type="radio" name="status" value="0" {{ $data['branch_find']->deleted_at == true ? $data['branch_find']->deleted_at == 0 ? "checked" : ""  : "checked"  }}/>
                        <label class="drinkcard-cc bg-success" for="status1"> เปิดการแสดงผล </label>  

                        <input id="status2" type="radio" name="status" value="1" {{ $data['branch_find']->deleted_at == 1 ? "checked" : "" }}/>
                    ...
                    <div class="col-md-12 form-group text-right">   
                      ...
                      @if(session('session_close'))
                        @if(session('session_close')=="Y")
                          <button type="button" class="btn btn-lg btn-danger waves-effect waves-light" id="close" data-id="{{ $data['branch_find']->id }}"><i class="fe-trash"></i> ยกเลิกข้อมูล </button>
                        @endif
                      @endif 
    ...
    Swal.fire({
        ...
        if (result.value) {  
          ...
          .done(function(data, status, error){   
            if(error.status==200){     
              if(data==true){
                location.href = "{{ route('branches.list') }}";
              } 
            }
          })
