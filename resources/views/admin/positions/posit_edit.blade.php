          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-header" style="background: #ddd;">  
                <div class="row">   
                  <div class="col-md-6"> 
                    <div class="m-0 h5"> แก้ไขข้อมูลตำแหน่ง </div>
                  </div>
              ...
              <div class="card-body">  
                ...
                <form method="POST" action="{{ route('save.positions') }}" id="form" enctype="multipart/form-data">
                  ...
                  <input type="hidden" id="id" name="id" value="{{ $data['position_find']->id }}">
                  ...
                  <div class="row"> 
                      ...
                      <select id="departments_id" name="departments_id" class="form-control mb-1 @error('departments_id') invalid @enderror" data-toggle="select2" required>
                        ...
                        @if(isset($data['Query_department']))
                          @foreach($data['Query_department'] as $row)
                            <option @if($data['position_find']->departments_id==$row->id) {{ __('selected') }} @endif value="{{ $row->id }}"> {{ $row->name }} </option>
                          @endforeach
                        @endif 
                      </select>   
                    ...
                    <div class="col-md-6 form-group"> 
                      ...
                      <div class="cc-selector"> 
                        <input id="status1" type="radio" name="status" value="0" {{ $data['position_find']->deleted_at == true ? $data['position_find']->deleted_at == 0 ? "checked" : ""  : "checked"  }}/>
                        <label class="drinkcard-cc bg-success" for="status1"> เปิดการแสดงผล </label>  

                        <input id="status2" type="radio" name="status" value="1" {{ $data['position_find']->deleted_at == 1 ? "checked" : "" }}/>
                    ...
                    <div class="col-md-12 form-group text-right">   
                      ...
                      @if(session('session_close'))
                        @if(session('session_close')=="Y")
                          <button type="button" class="btn btn-lg btn-danger waves-effect waves-light" id="close" data-id="{{ $data['position_find']->id }}"><i class="fe-trash"></i> ยกเลิกข้อมูล </button>
                        @endif
                      @endif 
...
<script> 
  ...
  seleted_sub_departments("{{ $data['position_find']->departments_id }}", "{{ $data['position_find']->sub_departments_id }}");
    ...
    Swal.fire({
        ...
        if (result.value) {  
          ...
          .done(function(data, status, error){   
            if(error.status==200){     
              if(data==true){
                location.href = "{{ route('positions.list') }}";
              } 
            }
          })
          .fail(function(xhr, status, error) { 
