          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-header" style="background: #ddd;">  
                <div class="row">  
                  <div class="col-md-2"> 
                    <select id="departments_id" name="departments_id" class="form-control mb-1" data-toggle="select2">
                  ...
                  <div class="col-md-2"> 
                    <select id="sub_departments_id" name="sub_departments_id" class="form-control mb-1" data-toggle="select2">
                  ...
                  <div class="col-md-3 text-right"> 
                    ...
                    <a href="{{ route('positions.add') }}" class="btn btn-primary waves-effect waves-light"><i class="fe-plus-square"></i> เพิ่ม </a>  
                  </div>
                </div>
              </div>
              <div class="card-body">  
                <div class="h5"> ตารางข้อมูลตำแหน่งประจำวันที่ {{ date('m/d/Y') }} </div> 
                <div class="table-responsive"> 
                  <table class="table table-borderless dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;" id="tbl-positions">
                    <thead>
                      <tr> 
                        <th style="width: 1%">  #รหัส </th>     
                        <th style="width: 20%;"> ชื่อตำแหน่ง </th> 
                        <th style="width: 20%;"> แผนกย่อย </th>   
...
<script> 
    ...
    Swal.fire({
        ...
        if (result.value) {  
          ...
          .done(function(data, status, error){   
            if(error.status==200){     
              if(data==true){
                $('#tbl-positions').DataTable().destroy();
                datatable(null, null);
              } 
            }
          })
  ...
  $(document).on('click', '#search', function(event) { 
    ...
    $('#tbl-positions').DataTable().destroy();
    datatable(keywrod, status, departments_id, sub_departments_id);
  });
  ...
  $(document).on('click', '#reset', function(event) { 
    ...
    $("select").val("").change();
    $('#tbl-positions').DataTable().destroy();
    datatable(null, null, null, null);
  });

  datatable();
  function datatable(keywrod, status, departments_id, sub_departments_id){  
    var table = $('#tbl-positions').DataTable({
      "processing":false,  
      "serverSide":false,  
      "searching": false,
      "lengthChange": true,  
      ajax: {
        url:"{{ route('datatable.positions') }}", 
        data:{
          keywrod: keywrod, 
          status: status,
